...
function BS93Amer(parametro, s, k, T, r, q, sigma, CallPut, tipo_subyacente) {
  ...
  let b = tipo_subyacente === "accion" ? r - q : tipo_subyacente === "futuro" ? 0 : NaN;
  if (isNaN(b)) throw new Error("tipo_subyacente debe ser 'accion' o 'futuro'");
  ...
  switch (parametro) {
    case "prima": return f(s, k, T, r, b, sigma);
    case "delta": return (f(s + h, k, T, r, b, sigma) - f(s - h, k, T, r, b, sigma)) / (2 * h);
    case "gamma": return (f(s + h, k, T, r, b, sigma) - 2 * f(s, k, T, r, b, sigma) + f(s - h, k, T, r, b, sigma)) / (h * h);
    case "theta": return (f(s, k, T - h, r, b, sigma) - f(s, k, T, r, b, sigma)) / h;
    case "vega": return (f(s, k, T, r, b, sigma + h) - f(s, k, T, r, b, sigma - h)) / (2 * h) / 100;
    case "rho": return (f(s, k, T, r + h, b, sigma) - f(s, k, T, r - h, b, sigma)) / (2 * h) / 100;
    default: return "Parámetro inválido";
  }
}

function BS93AmericanOption(optionType, S, X, T, r, b, sigma) {
  return optionType === "put"
    ? BS93AmericanCall(X, S, T, r - b, -b, sigma)
    : BS93AmericanCall(S, X, T, r, b, sigma);
}
...
/**
 * Estima la volatilidad implícita bajo el modelo de Bjerksund & Stensland (1993) mediante bisección.
 * @param {number} s Precio del subyacente.
 * @param {number} k Strike.
 * @param {number} T Tiempo hasta vencimiento (en años).
 * @param {number} r Tasa libre de riesgo.
 * @param {number} q Tasa de dividendos o carry.
 * @param {number} prima Prima observada.
 * @param {string} CallPut "call" o "put".
 * @param {string} tipo_subyacente "accion" o "futuro".
 * @return Volatilidad implícita estimada o "NA".
 * @customfunction
 */
